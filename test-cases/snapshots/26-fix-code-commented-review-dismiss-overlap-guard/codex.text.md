...
7. Run the `resolve:` command shown above, substituting "$HEAD_SHA" with the pushed commit SHA.
...
9. CI needs time to run on the new push. Pick a fresh sleep/timeout between 30 seconds and 4 minutes, wait that long, then rerun `npx pr-shepherd 42` to recheck.
