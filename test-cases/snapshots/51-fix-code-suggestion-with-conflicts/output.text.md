...
5. If the command refuses for any reason, including an unsafe anchored range or nested/unbalanced suggestion fences, skip patch application and use the manual-edit step below. Do not retry the command.
...
10. When `build-suggestion-patch` refuses for any reason, do not apply the replacement block verbatim. Inspect the surrounding source and reviewer intent, then make the intended edit manually.
11. Commit any remaining changes and push the conflict resolution before review mutations.
