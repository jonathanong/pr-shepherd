...
5. If the command refuses because the suggestion does not safely fit GitHub's anchored range, skip patch application and use the manual-edit step below. Do not retry the command.
...
10. When `build-suggestion-patch` refuses an unsafe anchored range, do not apply the replacement block verbatim. Inspect the surrounding source and reviewer intent, then make the intended edit manually.
