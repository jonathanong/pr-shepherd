...
1. Stack #50 in `owner/repo` is mergeable through PR #501. Run `GH_REPO=owner/repo gh stack merge --yes --squash 50` to merge the whole native stack or enqueue it when the base uses a merge queue.
