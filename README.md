...
4. If you changed code, commit and push the changes, then iterate again; do not run review mutations until the remote PR head reflects your push. If you did not change code, do not commit and continue.
...
6. If you did not change code, replace `$HEAD_SHA` with `$(git rev-parse HEAD)`, which must equal the current remote PR head. If you changed code, push it first and use its SHA.
...
9. `[FIX_CODE]` is conditional: after changed code, push and iterate again once the remote PR head reflects it; without code changes, complete the authorized review mutations and iterate again.
```
