...
`FIX_CODE` is always non-terminal: the caller completes the work, commits and pushes when needed, runs the capability-filtered review mutations Shepherd generated, and invokes Shepherd again. Only `ESCALATE` hands work to a human; `CANCEL` ends polling without a handoff.

Denied or unverifiable review replies, thread resolutions, and bot-review dismissals are one-look skips, not handoffs: Shepherd surfaces/logs the item once, omits the generated mutation, excludes it from fix-attempt accounting, and suppresses it until edited. Active threads without a path or line follow the same skip only when no authorized mutation remains. Authorized reply/resolve mutations still run by thread ID after GitHub clears the source location, and unlocated threads do not count toward `fix-thrash`. An ID omitted from automation is not prohibited: a user-directed `apply review` forwards every supplied reply, resolve, minimize, and dismiss ID without author, capability, or current-state filtering, and GitHub's mutation response is authoritative. `authorization-required` is reserved for attempted mark-ready and merge/enqueue state changes; the finite trigger list is documented in [`docs/escalations.md`](docs/escalations.md).
...
**Invariant-procedure exception:** a step whose *text* is byte-identical on every invocation belongs inline in the skill under `## Playbooks`, which loads it once per session, rather than in `## Instructions`, which would repeat it every tick. The test is whether the printed command/instruction stays safe and complete if the caller acts on it **exactly as printed, without the skill's guidance**: dismiss-ID retention passes this test (the printed command run unmodified is already correct; removing an emitted dismissal breaks it), so it stays in the skill. Placeholder substitution (`$HEAD_SHA`, `$DISMISS_MESSAGE`) fails it — the printed command is invalid without substitution — so those steps stay in `## Instructions` even though their text never varies. Marker-based self-reply prevention and first-look/check-annotation exclusion are already reflected directly in the CLI-generated IDs and need no skill prohibition or repeated instruction. An ID omitted from an automated command is not forbidden; a separate user-directed `apply review` may supply it. The CLI still owns the *trigger* and the *concrete generated command*, so a consumer without the skill can act correctly. Skills must not link to files outside `plugins/pr-shepherd/`, so moved content is written inline in `SKILL.md`, never linked to `docs/`.
