...
describe("runPoll — GraphQL quota throttle", () => {
  ...
  it("refreshes a reused MARK_READY when the next sleep would exceed timeout", async () => {
    mockRunIterate
  ...
  it("sleeps another MARK_READY interval when the bounded timeout still fits", async () => {
    mockRunIterate
