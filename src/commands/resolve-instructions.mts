...
export function buildFetchInstructions(
  ...
  instructions.push(
    `For any large decisions or rejections you made this iteration, add or update a \`## Shepherd Journal\` section in the PR description (\`gh pr edit ${prNumber} --body …\`) summarizing each decision and linking back to the originating comment, thread, or review using the markdown link shown in each item's bullet above.`,
  );
