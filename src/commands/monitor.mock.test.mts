...
describe("runMonitor", () => {
  ...
  it("uses configured package runner in reusable commands", async () => {
    vi.mocked(loadConfig).mockReturnValue({
      watch: { interval: "4m", readyDelayMinutes: 10 },
      cli: { runner: "pnpm" },
    } as unknown as PrShepherdConfig);
    const result = await runMonitor({ format: "text", prNumber: 42, runtime: "codex" });
    expect(result.reusableCommand).toBe("pnpm exec pr-shepherd 42");
    expect(result.loopPrompt).toContain("pnpm exec pr-shepherd 42");
  });
  ...
  it("builds a Codex prompt without /loop or Cron instructions", async () => {
    ...
    expect(result.loopPrompt).toContain(
      "pick a fresh sleep/timeout between 1 and 4 minutes, wait that long",
    );
