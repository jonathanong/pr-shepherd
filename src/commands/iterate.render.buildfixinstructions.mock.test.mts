...
describe("buildFixInstructions", () => {
  ...
  it("treats changes-requested reviews as review-only when no threads/checks exist", () => {
    ...
    expect(text).toContain("If you changed code, commit and push the changes");
    expect(text).toContain("do not run the remaining review mutations until the remote PR head");
    ...
    expect(text).toContain("`[FIX_CODE]` is conditional: if you changed code, commit and push it");
