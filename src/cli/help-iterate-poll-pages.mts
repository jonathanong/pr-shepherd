...
Unchanged WAIT snapshots are hidden by default; poll.quietStatus can change that default, --quiet-status/--no-quiet-status override it, and --verbose emits detailed per-tick lines.
FIX_CODE debounce writes a remaining-seconds line to stderr unless quiet status is enabled. --timeout does not cut an in-flight debounce short.
