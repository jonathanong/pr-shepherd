...
describe("triageFailingChecks — log boilerplate filter", () => {
  it("strips GitHub Actions runner setup lines and keeps the real error", async () => {
    const boilerplate = [
      "2024-01-01T00:00:00.0000000Z ##[group]Run actions/checkout@v6",
      "2024-01-01T00:00:00.1000000Z [command]/usr/bin/git checkout",
      "2024-01-01T00:00:01.0000000Z Cache hit for key: node-modules-abc123",
      "2024-01-01T00:00:02.0000000Z Download action repository 'actions/setup-node@v6'",
      "2024-01-01T00:00:03.0000000Z Found in cache @ /home/runner/tools/node/22.0.0",
      "2024-01-01T00:00:04.0000000Z Received 0 of 5 (0%), 0.00 MBs",
      "2024-01-01T00:00:05.0000000Z with:",
      "2024-01-01T00:00:06.0000000Z env:",
      "2024-01-01T00:00:07.0000000Z Run actions/cache@v4",
      "2024-01-01T00:00:08.0000000Z Removing includeIf entries",
      "2024-01-01T00:00:09.0000000Z Initialized empty Git repository",
      "2024-01-01T00:00:10.0000000Z Cleaning up orphan processes",
    ];
    const realError =
      "2024-01-01T00:00:11.0000000Z ##[error]src/foo.ts: error TS2345: argument type mismatch";
    const logText = [...boilerplate, realError].join("\n");

    mockFetch
      .mockResolvedValueOnce(makeJobsResponse([{ id: 1, name: "tests", conclusion: "failure" }]))
      .mockResolvedValueOnce(makeLogsRedirectResponse("https://blob.example.com/logs.txt"))
      .mockResolvedValueOnce(makeLogTextResponse(logText));

    const [result] = await triageFailingChecks([makeCheck()], REPO, 80);
    expect(result!.logTail).toBeDefined();
    expect(result!.logTail).toContain("error TS2345");
    // All the boilerplate lines should be absent.
    expect(result!.logTail).not.toContain("##[group]");
    expect(result!.logTail).not.toContain("[command]");
    expect(result!.logTail).not.toContain("Cache hit");
    expect(result!.logTail).not.toContain("Download action repository");
    expect(result!.logTail).not.toContain("Found in cache @");
    expect(result!.logTail).not.toContain("Run actions/");
  });

  it("preserves compiler diagnostics that resemble boilerplate prefixes", async () => {
    const lines = [
      "2024-01-01T00:00:00.0000000Z note: see previous errors",
      "2024-01-01T00:00:00.1000000Z   hint: try running `cargo fix`",
      "2024-01-01T00:00:00.2000000Z   KEY=value  (env dump from test runner)",
    ];
    ...
    const [result] = await triageFailingChecks([makeCheck()], REPO, 80);
    // These should NOT be filtered (they look like compiler output, not Actions boilerplate).
    expect(result!.logTail).toContain("note: see previous errors");
    expect(result!.logTail).toContain("hint: try running");
    expect(result!.logTail).toContain("KEY=value");
  });
});
