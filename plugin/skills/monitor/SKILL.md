...
- `[FIX_CODE]` → follow the numbered items under `## Instructions` in order. Two variants:
  - **Commit-suggestions shortcut** — if the output contains `## Commit suggestions`, every actionable thread carries a parseable ` ```suggestion ` block and the CLI is delegating to its `commit-suggestions` subcommand. Extract the backticked command from the `- commit-suggestions:` bullet and run it, then run the backticked `git pull --ff-only` from the `- after:` bullet. No rebase, no force-push, no `resolve:` ceremony — `commit-suggestions` creates the commit server-side and resolves the threads atomically. Stop this iteration afterward so CI can run.
