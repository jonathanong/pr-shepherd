...
2. **Fetch comments:**
   ...
   ```bash
   $([ -f dist/index.mjs ] && echo "node dist/index.mjs" || echo "npx pr-shepherd") resolve <N> --fetch --last-push-time "$LAST_PUSH" --format=json
   ```
...
6. **Resolve all verified items** — **only after the push:**
   ...
   ```bash
   $([ -f dist/index.mjs ] && echo "node dist/index.mjs" || echo "npx pr-shepherd") resolve <N> \
