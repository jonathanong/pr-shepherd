...
marker to local state — one file per id, its existence the marker itself:

```
$PR_SHEPHERD_STATE_DIR/<owner>-<repo>/<pr>/seen/<id>.json
```

The marker's JSON body is intentionally minimal and intentionally open-ended:

```json
{ "seenAt": 1732000000, "bodyHash": "3f9a1c2b7e4d5a6f" }
```
