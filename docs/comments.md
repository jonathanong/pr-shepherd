...
Classification-rule matches with both `suppress: true` and `autoResolve: true` are handled earlier when [`actions.autoMinimizeSuppressed`](configuration.md#actionsautominimizesuppressed--default-true) is enabled and GitHub reports the exact per-object capability: Shepherd resolves suppressed review threads and minimizes suppressed PR comments or COMMENTED review summaries before they become `fix_code` work. Confirmed threads land on `threads.autoResolved` (`isResolved: true`, plus `ruleReason` when the rule set `reason`). Confirmed comments and review summaries land on `comments.autoMinimized` as `{ id, url?, kind, ruleReason? }` — review summaries use `kind: "review-summary"` and include `url` when the batch query returned one. The same tick prints `## Classification auto-resolve` and appends one Shepherd Journal item. IDs GitHub did not confirm stay on the `apply review` handoff, with the error under that section. A journal failure is reported there and does not undo the mutation. Denied or unverifiable items remain visible through the normal first-look/edit gate and never enter a mutation command. `actions.autoMinimizeSuppressed: false` keeps that handoff and does not mutate, journal, or print the line.
...
Outdated threads are fetched from `batch.mts` and normally surface under `report.threads.resolutionOnly` until GitHub reports `isResolved: true`. Seen markers suppress repeated first-look/body display, but they do not suppress unresolved outdated/minimized threads from mutation routing. `resolutionOnly` also includes an active marker-ended thread that is still being resolved so a prior reply can be retried. Unmarked bot/non-human and viewer-authored human threads are paired in `--reply-thread-ids` and `--resolve-thread-ids`; already-marked ones are resolve-only for retry. An unmarked other-human thread remains reply-only unless [`iterate.resolveOtherHumanThreads`](configuration.md#iterateresolveotherhumanthreads) is `outdated` or `always`. Reply and resolve mutations use the thread ID, including when GitHub has cleared the obsolete path or line.

Outdated threads are surfaced and routed according to their current author and resolution state. Other-human resolve is controlled by [`iterate.resolveOtherHumanThreads`](configuration.md#iterateresolveotherhumanthreads).
...
- Unresolved outdated or minimized threads, plus active marker-ended retry threads that are still being resolved, in `report.threads.resolutionOnly`; bot/non-human and viewer-authored human threads are paired for reply-and-resolve unless their latest reply is marked, unmarked other-human threads receive a reply only unless `iterate.resolveOtherHumanThreads` allows resolve, and marker-ended other-human threads are already acknowledged at the default `none` setting.
...
Active other-human threads are suppressed after their transcript is seen unless the transcript changes, except when `iterate.resolveOtherHumanThreads` is `always`. Active bot/non-human and viewer-authored threads are returned every tick until resolved so Shepherd can keep asking the agent to finish the generated mutation. After evaluating a thread, the agent runs the generated review mutation even when the feedback is advisory, already satisfied, or otherwise warrants no code change. Unmarked bot/non-human and viewer-authored threads are reply-and-resolve; unmarked other-human threads remain reply-only unless the enum allows resolve. Shepherd prefixes every thread reply with `<!-- pr-shepherd -->`. That leading marker—not author equality—identifies a Shepherd reply, prevents another reply, and permits a later resolve-only retry if an earlier batch resolved unsuccessfully.
