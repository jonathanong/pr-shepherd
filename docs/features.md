...
- Compact, read-only aggregate polling for explicit same-repository PR lists and complete native GitHub stacks, with one-PR READY receipts, parent-first draft readiness, raw ancestry detection, and an agent-run bottom-layer merge command when requested and that layer is ready.
