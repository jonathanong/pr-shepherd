...
- Compact, read-only aggregate polling for explicit same-repository PR lists and complete native GitHub stacks, with one-PR READY receipts, parent-first draft readiness, raw ancestry detection, and an agent-run whole-stack merge command when requested and fully ready.
