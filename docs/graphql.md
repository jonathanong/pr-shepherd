...
- CI check runs (paginated forward, see below) and `checkSuites` (first 50, used for startup-failure detection). Each `CheckRun` includes an `annotations(first: 1)` probe so later annotation pagination runs only for checks that have at least one annotation. The probe stays `first: 1`: it only sets `hasAnnotations` and is part of the READY-receipt fingerprint. Annotation bodies use a separate batched query (see the operation catalog).
...
| `PollStackSummary`           | `poll-stack-summary.gql`                      | `--stack` summary; entry pages sized to the stack (`first: min(size, 50)`). A resource-limit error halves `first` down to 1 and continues with `after` | yes                      |
...
`annotations(first: 1)` on `BatchPr`, `BatchPrPage`, and `CommitCheckContexts` only detects whether a check has annotations. It is not the body fetch. The always-on poll-summary fragment does not select it. `PollSummaryAnnotationProbe` loads those totals for one commit only when that layer otherwise looks ready, paging with `before` past the newest 100 contexts, so a late annotation still changes the READY-receipt fingerprint. A failed or incomplete probe does not write a fingerprint and does not clear a receipt whose head and base still match. A check page past the first 100 contexts stays 1 point and does not repeat the probe.
...
| CI failing (one workflow run)                                                                           | those plus `CheckRunAnnotationsBatch` (20 checks per request) and `CheckRunAnnotations` only for `hasNextPage`                                  | 1 jobs list (more if >100 jobs) + optional log excerpt |
...
| Non-root native-stack layer (any tick, including fingerprint hits)                                      | plus 1× `PollStackTopology` per 50 stack entries                                                                                                | none                                                   |
| `--stack` summary                                                                                       | 1× `PollStackTopology`, then `PollStackSummary` pages sized to the stack. A resource-limit error halves that page down to one entry and rereads | none                                                   |
...
- Annotation bodies for uncached probe-positive checks use `CheckRunAnnotationsBatch` (20 check runs per request; 21 connection-requests, calculated cost 1 point) and are cached for 1 hour per check-run id. A rate-limit error stops the remaining chunks instead of logging one line per check.
