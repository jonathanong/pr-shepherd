...
- **`check-follow-up-unavailable`** — the only remaining blockers are failing checks without usable evidence or an authorized GitHub Actions rerun. `escalate.checks[]` preserves the raw check details so a human can inspect or operate the external/manual system. If other autonomous work exists, Shepherd completes that work under `[FIX_CODE]` first and reevaluates on the next tick.
