...
7. `## Changes-requested reviews` — marker-gated `CHANGES_REQUESTED` reviews. Each entry is emitted once, then suppressed until the body changes. Edited reviews are emitted again with `edited: true` in JSON. Bot/non-human CR reviews (`authorType` is `Bot`, `Unknown`, or a `[bot]`-suffix login, or a login in `botUsernames`) are automatically dismissed via `--dismiss-review-ids` in the `resolve:` command after the agent pushes a fix. Human-authored CR reviews are never auto-dismissed — the reviewer must re-review or dismiss themselves.
