...
9. `[FIX_CODE]` is conditional when review mutations require the PR head: after changed code, stop until an authorized push changes the remote head; without code changes, complete the authorized mutations and iterate again. Failing checks and conflicts can also require terminal authorization handoffs.
```
...
5. `## Failing checks` — one bullet per failing check. Shape varies by locator:
   ...
   Every bullet carries a `[conclusion: <CONCLUSION>]` tag (e.g. `[conclusion: FAILURE]`, `[conclusion: TIMED_OUT]`, `[conclusion: CANCELLED]`, `[conclusion: STARTUP_FAILURE]`); null conclusions produce no tag. A check with a run ID additionally carries a `[rerun authorized]` tag and a `  rerun: \`gh run rerun <runId> -R <owner/repo>\``sub-line when the viewer's`repositoryPermission`is`WRITE`/`MAINTAIN`/`ADMIN`— the exact repository role GitHub's Actions rerun API requires. External checks (no run ID) never carry this tag. Non-CANCELLED bullets may also carry a`> <failedStep>`blockquote line (the first step that failed, GitHub Actions only), a`> <summary>`blockquote line (one-line status text from the GitHub UI), and a bounded`> <logExcerpt>`blockquote from the matched failed job log. For aggregate jobs that print`Job results`, `logExcerpt` is condensed to non-success job results plus the exit-code/error line; otherwise it is a bounded raw excerpt. All three are omitted when not available.
...
`fix.checks[].rerunCommand` is present only when the check has a `runId` and the viewer's `repositoryPermission` is `WRITE`/`MAINTAIN`/`ADMIN`; both Markdown (`[rerun authorized]` tag + `rerun:` sub-line) and JSON expose the same value. Shepherd never runs the rerun itself — the CLI only recommends the `gh run rerun` command for the agent to execute, matching the `commit-suggestion` pattern used elsewhere for git mutations.
