...
- `--require-sha "$HEAD_SHA"` is appended when a mutation may become stale on a moving HEAD: specifically, when actionable thread resolutions are included, failing checks are being addressed, or `--dismiss-review-ids` is present. Resolution-only threads in `--resolve-thread-ids` do not trigger this on their own. Comment-only and summary-only dispatches omit it.
