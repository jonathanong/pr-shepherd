...
11. `## In-progress runs` — backticked run IDs of CI checks that are still running. Emitted only when `fix.inProgressRunIds` is non-empty. The agent cancels these before applying code fixes (step 1 of `## Instructions`). Distinct from `## Cancelled runs`: those IDs were already cancelled by the CLI before the agent acts; these IDs the agent must cancel itself now. Not emitted when empty.
