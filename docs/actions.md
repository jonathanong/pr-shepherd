...
Required status contexts that never produced a check run or status context are their own `FIX_CODE` cause, including when other checks passed and `mergeStateStatus` is `BLOCKED`. For a native stack the required names come from the stack trunk (`stack.baseRefName`), not the layer's parent branch. The newest run of a name wins, so a later success supersedes an older cancelled run. Shepherd stays on `WAIT` while any relevant check run is in progress or a check suite with a `workflowRun` is not completed. Suites with no workflow run that stay `QUEUED` do not count. The instruction names the missing contexts. If the stack trunk compare is behind or this PR's derived merge status is `BEHIND`, it rebases onto the trunk with `gh stack rebase` (a non-stack PR updates from its base) and pushes. Otherwise it runs `gh pr close <pr> -R <repo>` and `gh pr reopen <pr> -R <repo>` once for this head. The next tick on that same head, still missing those contexts with nothing running, returns `ESCALATE` with `required-checks-unreported`. Failing checks and merge conflicts keep their existing paths. Text prints `**unreported required**` and, when the trunk is behind, `**trunk behind**`. JSON uses `unreportedRequiredChecks` and `trunkBehindBy`, omitted when empty. A `--stack` summary starts a one-PR session (`unreported-required-checks`) instead of idling in `pending-or-unknown`.
...
- **`required-checks-unreported`** — required merge-target contexts still have no check run and no status context after one close/reopen of this head, and no Actions workflow is running. Another reopen will not create a job the workflow does not emit. Path filters are the usual cause. A repeated rebase for a behind trunk stays on `FIX_CODE` until the ordinary `stall-timeout` timer fires. See [`required-checks-unreported`](escalations.md#required-checks-unreported).
