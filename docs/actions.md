...
3. `## Review threads to resolve` — unresolved outdated/minimized inline threads that should be passed to `--resolve-thread-ids` but do not require code edits unless the agent chooses to act on the body. Each item includes the full comment transcript, not a preview.
...
7. `## Changes-requested reviews` — one bullet per `CHANGES_REQUESTED` review: ``- `reviewId=<id>` (@<author>[ · <authorType>])``.
