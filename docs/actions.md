...
**Held native stack drafts:** a draft native stack layer that this one-PR session cannot promote carries `stackDraftHold` in JSON — `auto-mark-ready-disabled` (the session or configuration disables automatic mark-ready, as in the stack selector's bounded draft probes) or `lower-layer-not-ready` (a `READY` upper draft whose lower layer lacks a current READY receipt). Repeating the same one-PR session cannot advance it, so the single instruction replaces "iterate immediately" with a stack handoff (quota cadence advice is appended when a warning applies):
