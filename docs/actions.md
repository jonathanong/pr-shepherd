...
The default CLI command owns its `--interval`/`--timeout` waits. A final `WAIT` returned at timeout is still non-terminal, so the skill starts another bounded poll. MCP `iterate` and `pr-shepherd iterate` return one tick and their caller schedules the next one — the instruction text itself no longer names a transport; it stays terse ("rerun this command") because the pr-shepherd skill's dispatcher step already owns the loop for both CLI and MCP callers (see [`plugins/pr-shepherd/skills/pr-shepherd/SKILL.md`](../plugins/pr-shepherd/skills/pr-shepherd/SKILL.md)). A `--ready-delay 15m` override remains a summary field rather than part of a rerun command; JSON carries the same value as `readyDelayOverride`.
...
5. Run the `apply review:` command shown above.
6. `[FIX_CODE]` is non-terminal. After completing these steps, rerun this command to continue.
```

The CLI surfaces the raw `**branch**` state on the summary line and leaves rebase/commit/push conventions to the caller. Steps that were previously spelled out per invocation — Shepherd Journal citation conventions, `$HEAD_SHA`/`$DISMISS_MESSAGE` substitution, the self-reply exclusion rule — are invariant text, so they now live in the pr-shepherd skill's `## Playbooks` section (see [`plugins/pr-shepherd/skills/pr-shepherd/SKILL.md`](../plugins/pr-shepherd/skills/pr-shepherd/SKILL.md)) instead of being re-emitted every tick; the CLI keeps only the trigger and the concrete command. The final FIX_CODE instruction no longer names a transport — it reads "rerun this command" regardless of caller, because the pr-shepherd skill's dispatcher step already owns the loop for both CLI and MCP callers. A bare failing check with neither a run ID nor a URL instead ends with a human-handoff instruction that pauses polling until human direction.
...
5. `## Failing checks` — one bullet per failing check. Shape varies by locator:
   ...
   The numbered instructions emit one triage pointer whenever any check has a run ID or a `detailsUrl` — "Triage every failure under `## Failing checks`… See 'CI failure triage' in the pr-shepherd skill for `gh run view` / `gh run rerun` rules." The per-conclusion table (runId → read/fetch logs and rerun or fix; CANCELLED → rerun unless a same-tick push supersedes it, never silently treat as resolved; STARTUP_FAILURE → inspect metadata and rerun; external URL → inspect it) lives in that skill playbook, keyed on the `[conclusion: …]` tag each bullet already carries. Only the no-run-ID-or-URL case still gets its own CLI instruction ("escalate to a human because no log or URL is available"), since it flips the tick to a human-handoff terminal state. Both entries are omitted when no failing checks exist.
