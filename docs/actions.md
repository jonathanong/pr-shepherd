...
When a configured GraphQL quota threshold is crossed on a non-terminal result, lean Markdown and JSON include a `GitHub API quota warning` / `quotaWarning` block. The Markdown block includes `Recommended poll interval`, `Recommended bounded CLI timeout`, and a `Recommendation` line. The final instruction replaces the ordinary immediate continuation: keep polling pr-shepherd at that cadence; for incidental PR operations that do not need Shepherd's full snapshot, prefer REST `gh` (`gh pr view`, `gh pr review`, `gh api` REST endpoints), since GraphQL and REST draw from separate quota pools. Do not substitute `gh pr checks` or `gh pr watch` for the Shepherd loop — those waiters only see CI and hide review comments until checks finish. Resume full-cadence pr-shepherd once the GraphQL quota resets (the block's `Reset` time). If the caller must keep polling before then, it falls back to a minimum poll interval: a bounded polling CLI command replaces existing `--interval` and `--timeout` flags; the shipped skill preserves `--until-terminal`, omits `--timeout`, and adjusts its cadence before re-invocation; a single-tick CLI, API, or MCP caller waits before its next tick instead. The warning is emitted once per worktree (or process session when no worktree is available) and quota window. The poll dispatcher also applies `max(--interval, band interval)` on every `WAIT` / `MARK_READY` sleep from the latest GraphQL remaining percent, including after the one-shot warning has already been claimed. An unbounded `--until-terminal` poll still returns the first warning so the skill can slow incidental GraphQL and re-invoke with `--interval`. Single-tick `iterate` / MCP do not sleep. Terminal `cancel`/`escalate` results do not warn because no automated continuation is actionable; every `[FIX_CODE]` result remains eligible because it is non-terminal. GraphQL usage, cost, and fingerprint skip: [graphql.md](graphql.md).
...
5. Run the generated thread IDs unchanged. A latest comment beginning `<!-- pr-shepherd -->` is an established Shepherd reply: a marked thread that is still being resolved is emitted resolve-only.
...
- A leading `<!-- pr-shepherd -->` in the latest visible comment is the only signal for a prior Shepherd reply; author equality is not sufficient. The generated command does not re-reply to a marked thread. An unmarked bot/non-human or viewer-authored human thread may appear in both reply and resolve flags. Direct unmarked other-human resolve without that paired reply is skipped unless `iterate.resolveOtherHumanThreads` allows it. A marked other-human thread is already acknowledged at the default `none` setting.
