...
2. `## Review threads` — marker-gated active unresolved threads. Each thread appears under ``### `threadId=<id>` — <loc> (@author) [suggestion]?`` (or `### [threadId=<id>](<url>) — <loc> (@author) [suggestion]?` when a URL is available) where `<loc>` is `` `path:line` `` for single-line threads or `` `path:startLine-endLine` `` for multi-line threads. The thread's full comment transcript follows, with each comment/reply rendered under ``#### `commentId=<id>` (@author)`` (or linked when a comment URL is available) and the full body as a `>` blockquote. Multi-paragraph bodies preserve empty lines as `>` lines. Threads with a ` ```suggestion ` fence in the top comment carry a `[suggestion]` tag in the thread heading and a `Replaces lines …` block after the transcript showing the parsed replacement. A previously seen thread whose transcript changed is rendered again with `[edited since first look]`; unchanged seen threads are suppressed until markers are cleared.
3. `## Review threads to resolve` — marker-gated unresolved outdated/minimized inline threads that do not require code edits unless the agent chooses to act on the body. Human-authored IDs are passed to `--reply-thread-ids`; Shepherd does not resolve or minimize them. Each item includes the full comment transcript, not a preview. A previously seen thread whose transcript changed is rendered again with `[edited since first look]`; unchanged seen threads are suppressed until markers are cleared.
...
15. `## Post-fix push`:
    ...
    - ``- resolve: `<argv>` `` — fully-quoted resolve command. `$DISMISS_MESSAGE` and `$HEAD_SHA` are always quoted so substituting a multi-word sentence keeps it as one argument. Human thread IDs use `--reply-thread-ids` and require `$DISMISS_MESSAGE`; bot/non-human thread IDs use `--resolve-thread-ids`. Shepherd does not resolve or minimize human-authored threads. `--require-sha "$HEAD_SHA"` is appended when the underlying resolve command can mutate state against a moving HEAD — specifically, when replies/resolves follow actionable thread fixes or when failing checks are being addressed. Reply-only resolution-only threads do not trigger `--require-sha`.
16. `## Instructions` — numbered list to execute in order. The final instruction always refers back to the `resolve:` bullet rather than duplicating the command — that single source of truth is what the skill executes.
...
- `--require-sha "$HEAD_SHA"` is appended when a mutation may become stale on a moving HEAD: specifically, when `--reply-thread-ids` follows actionable thread fixes or when failing checks are being addressed. Reply-only resolution-only threads do not trigger `--require-sha`. Comment-only and summary-only dispatches omit it.
