...
14. `## Post-fix push`:
    ...
    - ``- resolve: `<argv>` `` — fully-quoted resolve command. `$DISMISS_MESSAGE` and `$HEAD_SHA` are always quoted so substituting a multi-word sentence keeps it as one argument. `--require-sha "$HEAD_SHA"` is appended when a resolve mutation could become stale on a moving HEAD (actionable threads, failing checks, CHANGES_REQUESTED reviews combined with actionable comments or resolution-only threads, or rebase conflicts).
...
- `--require-sha "$HEAD_SHA"` is appended when actionable threads, failing checks, CHANGES_REQUESTED reviews with actionable PR-level comments or resolution-only threads, or rebase conflicts are present — signals the CLI knows a resolve mutation should be tied to the current PR SHA. Comment-only and summary-only dispatches omit it.
