...
**Trigger:** Any of: unresolved inline review threads, resolution-only inline review threads, actionable PR-level comments, `CHANGES_REQUESTED` reviews, any failing CI check, unseen check-run annotations on non-passing checks, merge conflicts (`mergeStatus.status === "CONFLICTS"`), or pending first-look review summary IDs to minimize. Failing checks of all types (timeout, cancelled, startup failure, actionable) route here — the agent reads included log excerpts first, runs a bounded command such as `gh run view <runId> --log-failed | tail -n 200` when it needs fuller job logs (the unbounded form can dump excessive log output into context), and follows the CANCELLED/STARTUP_FAILURE per-conclusion rules for externally cancelled runs or jobs/logs that may not exist. That triage table lives in the pr-shepherd skill's "CI failure triage" playbook (see [`plugins/pr-shepherd/skills/pr-shepherd/SKILL.md`](../plugins/pr-shepherd/skills/pr-shepherd/SKILL.md)), keyed on the `[conclusion: …]` tag each bullet already carries — see section 5 below. Unseen check annotations on skipped, ignored, or filtered CheckRuns also route here for one tick unless their conclusion is `SUCCESS`; they do not keep `fix_code` alive after the seen marker is written. A successful parent CheckRun is authoritative for iteration regardless of classification bucket, so its annotations remain visible in the current `check` output and are marked seen without triggering `FIX_CODE`.
...
5. `## Failing checks` — one bullet per failing check. Shape varies by locator:
   ...
   The numbered instructions emit one triage pointer whenever any check has a run ID or a `detailsUrl` — "Triage every failure under `## Failing checks`. See 'CI failure triage' in the pr-shepherd skill for `gh run view` / `gh run rerun` rules." That sentence does not itself claim every failure has a log excerpt to read — only GitHub Actions checks with a runId do. The per-conclusion table (runId → read the included log excerpt, fetch/rerun or fix; CANCELLED → rerun unless a same-tick push supersedes it, never silently treat as resolved; STARTUP_FAILURE → inspect metadata and rerun; external URL → inspect it) lives in that skill playbook, keyed on the `[conclusion: …]` tag each bullet already carries. Only the no-run-ID-or-URL case still gets its own CLI instruction ("escalate to a human because no log or URL is available"), since it flips the tick to a human-handoff terminal state. Both entries are omitted when no failing checks exist.
...
16. `## Post-fix push`:
    ...
    - ``- resolve-only: `<argv>` `` — present when resolve/minimize mutations are split from the message-bearing reply/dismiss command. It can contain bot/non-human work or a marker-ended viewer-authored human resolve retry. Its instruction appears before `apply review:` and requires no substitutions. Omitted when all mutations are combined into the single `apply review:` line.
    - ``- apply review: `<argv>` `` — fully quoted apply command. `$DISMISS_MESSAGE` and `$HEAD_SHA` are always quoted so substituting a multi-word sentence keeps it as one argument. Unmarked other-human thread IDs use `--reply-thread-ids` only. An unmarked human inline thread whose original comment has `viewerDidAuthor: true` appears in both `--reply-thread-ids` and `--resolve-thread-ids`, in that order; direct unmarked human resolve requests without the paired reply are skipped. A viewer-authored human thread whose latest comment begins `<!-- pr-shepherd -->` is resolve-only for retry. A marker-ended other-human thread is already acknowledged and is not included in any mutation flag. Bot/non-human thread IDs use `--resolve-thread-ids` (or move to `resolve-only:` when split). Bot/non-human CR review IDs use `--dismiss-review-ids` in the same command as replies (both need `--message`); when dismiss mutations are split from resolve/minimize mutations, the dismiss IDs ride in the `apply review:` line, not `resolve-only:`. Shepherd does not minimize or dismiss human-authored reviews or threads, and it does not resolve another human's inline thread. The leading marker—not author equality—identifies a prior Shepherd reply. `--require-sha "$HEAD_SHA"` is appended when the command contains `--reply-thread-ids` following actionable thread fixes, when failing checks are being addressed, or whenever `--dismiss-review-ids` is present (dismissal is a post-push mutation).
...
`fix.inProgressRunIds` contains the GitHub Actions run IDs the agent should cancel before pushing (mirrors `## In-progress runs` in the Markdown output); populated only when there is plausible-push work (actionable threads, failing checks, `CHANGES_REQUESTED` reviews, actionable PR comments, or merge conflicts) AND at least one in-progress check has a non-null run ID not already cancelled by the CLI and not protected by `actions.neverCancelRuns`. Resolution-only and summary-only iterations set this to an empty array. External status checks, already-cancelled runs, and protected runs are excluded. In lean JSON mode this field is omitted when empty.

`fix.protectedRuns` contains workflow runs protected by `actions.neverCancelRuns`: `{ runId, matchedPattern, checkNames, workflowName? }`. A match on any workflow/check name protects the entire run ID because GitHub's cancellation API is workflow-run scoped. In lean JSON mode this field is omitted when empty.

`branchProtection` is `null` when no branch protection rule applies; otherwise it is `{ requiresApprovingReviews, requiredApprovingReviewCount, requiresConversationResolution, requiresStatusChecks, requiredStatusCheckContexts }` — the raw values from GitHub's `branchProtectionRule`. `resolutionOnlyThreads` contains unresolved outdated/minimized review threads plus active marker-ended viewer-authored human retry threads. For human threads, an unmarked viewer-authored original comment is paired in `--reply-thread-ids` and `--resolve-thread-ids`; a viewer-authored thread whose latest comment begins `<!-- pr-shepherd -->` is resolve-only for retry; an unmarked other-human thread remains reply-only; and a marker-ended other-human thread is already acknowledged and is suppressed from further mutation. Bot/non-human ones are routed to `--resolve-thread-ids` on every run until GitHub reports them resolved. `reviewSummaryIds` contains the non-human review IDs routed to `--minimize-comment-ids`: first-look COMMENTED summaries that pass `iterate.minimizeComments` and have no unresolved known inline child threads, plus any classification-rule `autoResolve` IDs not already self-applied, and APPROVED reviews only when approval minimization is enabled and they also pass the author policy. It does **not** include already-seen COMMENTED summaries — those are minimized in-process before `reviewSummaryIds` is computed (issue #313) and never surface to the agent, unless GitHub fails to confirm the mutation, in which case the ID is added back so it is not silently lost.
...
> For all threads marked `` `[suggestion]` `` under `` `## Review threads` ``, run one `` `pr-shepherd build-suggestion-patches 42 --thread-id <id> --message "<one-sentence headline>" --format=json` `` command, repeating the thread/message group in displayed order. Apply, stage, and commit the returned patches in order, then push once. If the command refuses because a suggestion is unsafe or the ordered patches do not apply to the current local descendant, inspect the surrounding source and reviewer intent and apply the intended change manually. Preserve the generated `apply review:` routing: viewer-authored human feedback can intentionally be paired for reply-and-resolve, while unmarked other-human feedback stays reply-only; marker-ended other-human feedback is already acknowledged and has no mutation.
