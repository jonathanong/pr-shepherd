...
14. `## Post-fix push`:
    ...
    - ``- resolve: `<argv>` `` — fully-quoted resolve command. `$DISMISS_MESSAGE` and `$HEAD_SHA` are always quoted so substituting a multi-word sentence keeps it as one argument. `--require-sha "$HEAD_SHA"` is appended when the underlying resolve command includes `--resolve-thread-ids`, `--dismiss-review-ids`, or failing checks to address (i.e., mutations likely to be stale on a moving HEAD).
