...
5. For each `[conclusion: STARTUP_FAILURE]` bullet under `## Failing checks`: the workflow failed before jobs/logs were created. Run `gh run view <runId>` to inspect the run metadata, then run `gh run rerun <runId>` if the workflow should be attempted again.
...
14. `## Post-fix push`:
    ...
    - ``- resolve: `<argv>` `` — fully-quoted resolve command. `$DISMISS_MESSAGE` and `$HEAD_SHA` are always quoted so substituting a multi-word sentence keeps it as one argument. `--require-sha "$HEAD_SHA"` is appended when the underlying resolve command can mutate state against a moving HEAD: when actionable thread IDs are included in `--resolve-thread-ids`, when `--dismiss-review-ids` is present, or when failing checks are being addressed. Resolution-only threads alone in `--resolve-thread-ids` do not trigger this.
...
- `--require-sha "$HEAD_SHA"` is appended when a mutation may become stale on a moving HEAD: specifically when actionable thread IDs are included in `--resolve-thread-ids`, when `--dismiss-review-ids` is present, or when failing checks are being addressed. Resolution-only threads alone in `--resolve-thread-ids` do not trigger this on their own. Comment-only and summary-only dispatches omit it.
