...
14. `## Post-fix push`:
    ...
    - ``- resolve: `<argv>` `` — fully-quoted resolve command. `$DISMISS_MESSAGE` and `$HEAD_SHA` are always quoted so substituting a multi-word sentence keeps it as one argument. `--require-sha "$HEAD_SHA"` is appended when a resolve mutation could become stale on a moving HEAD (actionable threads, CI failures, or CHANGES_REQUESTED reviews present); comment-only and summary-only dispatches omit it.
...
- `--require-sha "$HEAD_SHA"` is appended when actionable threads, CI failures, or CHANGES_REQUESTED reviews are present — signals the CLI knows a resolve mutation should be tied to the current PR SHA. Comment-only and summary-only dispatches omit it.
