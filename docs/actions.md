...
14. `## Post-fix push`:
    ...
    - ``- resolve: `<argv>` `` — fully-quoted resolve command. `$DISMISS_MESSAGE` and `$HEAD_SHA` are always quoted so substituting a multi-word sentence keeps it as one argument. `--require-sha "$HEAD_SHA"` is appended when the underlying resolve command includes actionable thread mutations (`--resolve-thread-ids` for non-resolution-only threads), `--dismiss-review-ids`, or failing checks to address (i.e., mutations likely to be stale on a moving HEAD).
...
- `--require-sha "$HEAD_SHA"` is appended when a mutation may become stale on a moving HEAD — specifically when actionable thread resolutions are included, `--dismiss-review-ids` is present, or there are failing checks to address — signals the CLI knows a resolve mutation should be tied to the current PR SHA. Resolution-only threads in `--resolve-thread-ids` do not trigger this on their own. Comment-only and summary-only dispatches omit it.
