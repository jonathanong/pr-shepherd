...
Instruction wording is agent-aware. Claude-compatible output refers to the next cron fire and `/loop cancel`. Codex output is selected with `AGENT=codex` or `CODEX_CI=1`; it replaces those lines with active-goal guidance such as waiting about the configured interval (`watch.interval`, default 4m) and rerunning the configured pr-shepherd command. The action data and section structure are otherwise the same.
...
14. For any large decisions or rejections you made this iteration, add or update a `## Shepherd Journal` section in the PR description (`gh pr edit 42 --body …`) summarizing each decision and linking back to the originating comment, thread, or review.
...
- In Codex output, the "next tick" variants are rewritten to tell Codex to continue the active goal, wait about the configured interval (`watch.interval`, default 4m), and rerun the configured pr-shepherd command.
