...
**Body truncation (Markdown only):** thread, actionable-comment, and review-summary bodies (sections 2–4, 8, 9, 11, and 12 above) are capped at 1,200 characters for a top-level body and 600 characters for a nested thread reply, keeping a head and a tail so an opening question and a trailing summary both survive. An elided body carries `[…N lines elided — full text: <url>]` in place of the missing middle (the link is omitted where no URL is available, e.g. review summaries). The cut point always snaps to a line boundary outside any ` ``` ` fence, so a capped body never leaves an unterminated fence — if no fence-safe cut exists (a single fence spanning the whole body), the body is left uncapped rather than risk corrupting every section rendered after it. Pass `--verbose` to render bodies uncapped. **JSON is never capped:** `body` fields always carry the full text regardless of format or `--verbose` — the Markdown cap is a presentation choice over the same fetched data, not a difference in what iterate retrieved, so a capped Markdown body's linked URL and the corresponding JSON `body` field always agree.
