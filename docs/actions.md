...
Instruction wording is agent-aware. Claude-compatible output schedules the next dynamic wakeup with `ScheduleWakeup` (`delaySeconds` 60-240) or stops without scheduling another wakeup for terminal states. Codex output is selected with `AGENT=codex` or `CODEX_CI=1`; it replaces those lines with active-goal guidance to pick a fresh sleep/timeout between 1 and 4 minutes before rerunning `npx pr-shepherd <PR>`. The action data and section structure are otherwise the same.
