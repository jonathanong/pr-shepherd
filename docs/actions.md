...
7. `## Review summaries (already surfaced — minimize queue)` — backticked review IDs (`PRR_…`) of `COMMENTED` review summaries whose bodies were surfaced in a **prior** iteration. If `iterate.minimizeApprovals` is `true`, this section may also include `APPROVED` review IDs queued for minimization even though their bodies were not previously surfaced. All IDs (from both sections 6 and 7) are merged into `--minimize-comment-ids` in the resolve command. Not emitted when empty.
