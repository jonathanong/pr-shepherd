...
- `--require-sha "$HEAD_SHA"` is appended only when threads or CI failures are present — signals the CLI knows a push is required. Comment-only and summary-only dispatches omit it.
