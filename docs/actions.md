...
9. `[FIX_CODE]` is non-terminal unless every failing check is a `CANCELLED`/`STARTUP_FAILURE` authorization-only handoff, or any failure has neither a run ID nor URL. In those handoff-only cases, stop polling and resume after human direction.
```
...
5. `## Failing checks` — one bullet per failing check. Shape varies by locator:
   ...
   The numbered instructions emit one triage pointer whenever any check has a run ID or a `detailsUrl` — "Triage every failure under `## Failing checks`. See 'CI failure triage' in the pr-shepherd skill for inspection rules." The playbook may recommend read-only inspection, but never cancellation or rerun because GitHub exposes no exact viewer capability for those actions. Only the no-run-ID-or-URL case gets its own CLI instruction ("escalate to a human because no log or URL is available").
...
`fix.inProgressRunIds` and `cancelled` remain empty. GitHub exposes no exact viewer capability for workflow-run cancellation, so neither Markdown nor JSON recommends it.
