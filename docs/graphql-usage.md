...
1. **Stop nesting annotation probes under both summary rollups.** `contexts(last: 100) { annotations(first: 1) }` is selected on the head commit and on `mergeQueueEntry` for every PR in the summary, including PRs that are not queued. The probe exists so [`fingerprintRawSummaryPr`](../src/github/poll-summary-fingerprint.mts) changes when a check gains an annotation. [`summarizePollSummaryChecks`](../src/github/poll-summary-checks.mts) does not read it. Dropping only the queue rollup reduces a 10-PR stack from 25 to 15 points per tick, or about 450 points/hour at the default interval. Removing both annotation probes reduces it to the measured 5-point query, or about 150 points/hour at 30 ticks. The production fragment's other connections add less than 1 point at that size.
