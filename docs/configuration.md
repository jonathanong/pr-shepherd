...
There is no config key to opt out of stack detection: when GitHub's batch query reports the PR is part of a native stack, singular Shepherd escalates instead of building a command at all, so `merge.commandArgs` is never consulted for that PR. The native `--stack` and `--stack --merge` selectors are reconciliation-only: unready layers return `FIX_CODE` with one-PR routing, queued stacks return `WAIT`, and terminal READY or merged stacks return `CANCEL`. Closed or unverified topology and a fully READY `--stack --merge` result return `ESCALATE` for human direction. The selectors never emit `gh stack merge` or other aggregate mutation commands. See [merge-status.md](merge-status.md#merge-requirements) and [escalations.md#stacked-pr](escalations.md#stacked-pr).
