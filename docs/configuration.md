...
There is no config key to opt out of stack detection: when GitHub's batch query reports the PR is part of a native stack, singular Shepherd escalates instead of building a command at all, so `merge.commandArgs` is never consulted for that PR. The native `--stack --merge` selector reconciles all READY receipts and uses the configured strategy only for its whole-stack command. See [merge-status.md](merge-status.md#merge-requirements) and [escalations.md#stacked-pr](escalations.md#stacked-pr).
