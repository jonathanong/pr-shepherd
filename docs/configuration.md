...
`intervalSeconds` and `timeoutSeconds` must be positive finite numbers. `stackIntervalFactor` must be a finite number greater than or equal to 1 (built-in `2`). `debounceSeconds` must be a non-negative finite number; `0` disables the post-`FIX_CODE` settle window. `quietStatus` defaults to `false`, preserving the ordinary status line for every `WAIT` tick. Use `--quiet-status` to hide unchanged snapshots for one invocation or `--no-quiet-status` to override a configured `true` value.
