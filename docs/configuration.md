...
There is no config key to opt out of stack detection: when GitHub's batch query reports the PR is part of a native stack, singular Shepherd escalates instead of building a command at all, so `merge.commandArgs` is never consulted for that PR. The native `--stack` and `--stack --merge` selectors are reconciliation-only: they return only `CANCEL` or `ESCALATE`, route unready layers to one-PR sessions, and never emit `gh stack merge` or other aggregate mutation commands. A fully READY `--stack --merge` result is handed to the stack owner for the merge decision. See [merge-status.md](merge-status.md#merge-requirements) and [escalations.md#stacked-pr](escalations.md#stacked-pr).
