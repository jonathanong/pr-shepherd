...
| `merge.method`                       | unset                                     | Preferred method: `merge`, `squash`, or `rebase`. Used only when the repository allows it. A strategy flag in `commandArgs` must name the same method.           |
| `merge.commandArgs`                  | `[]`                                      | Extra options for ordinary `gh pr merge` commands. Not used for merge-queue commands. A strategy flag here is the preferred method when `merge.method` is unset. |
...
`merge.method` is the preferred merge method. `merge.commandArgs` is appended only to ordinary auto/direct `gh pr merge` commands emitted by `--merge`. Shepherd rejects the PR selector, `--repo`/`-R`, auto-mode controls (`--auto`/`--disable-auto`), `--match-head-commit`, privilege bypass via `--admin`, help flags (`--help`/`-h`), and file-reading body options (`--body-file`/`-F`), including attached short-option values. Select at most one of `--merge`/`-m`, `--squash`/`-s`, or `--rebase`/`-r`; boolean assignments such as `--squash=true` and safe short boolean bundles such as `-sd` are recognized. That strategy flag and `merge.method` must name the same method. When neither is set, a one-PR command uses the first repository-allowed method in the order merge, squash, rebase, and a stack command prefers squash, then merge, then rebase. A configured method the repository disables is not suggested. When the repository enables no method, Shepherd reports that constraint instead of a command. Queue commands omit every configured option because the queue controls the merge method and does not accept branch deletion.
