...
| Tag / kind                            | Do                                                                                                                                                      |
| ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GitHub Actions failure (has a run ID) | Read the included log excerpt first. If insufficient, run `gh run view <runId> --log-failed`; open the run URL only if the API still lacks detail.      |
...
| `[conclusion: CANCELLED]`             | Run `gh run rerun <runId>` unless this tick will push new commits. Not resolved by a rerun classification — `## Cancelled runs` is a different section. |
| `[conclusion: STARTUP_FAILURE]`       | Inspect with `gh run view <runId>`, rerun with `gh run rerun <runId>` if warranted.                                                                     |
