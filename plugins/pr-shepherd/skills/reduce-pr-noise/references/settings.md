...
| An irrelevant check context               | `ignoreChecks`                                       | Removes detailed check status and readiness, triage, and stall effects; names remain in the `**ignored**` rollup. Patterns are case-insensitive globs; `actions.neverCancelRuns` can keep matching Actions runs visible. CodSpeed checks and Codecov "No coverage information found on base report" conclusions are already ignored without configuration. |
