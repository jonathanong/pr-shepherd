...
| ----------------------------------------- | ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Repeated unchanged `WAIT` status          | `poll.quietStatus` (default `true`) or `--quiet-status` | Already the default: hides unchanged polling snapshots. Use `--no-quiet-status` or `poll.quietStatus: false` to print every tick. Does not affect single-tick `iterate` or MCP calls.                                    |
...
For example, a project that wants to restore per-tick WAIT lines and to remove a known teardown line from CI excerpts can use:
